    ...
    <v-card class="p-5">
          ...
          <v-expansion-panel-content>
            <div class="row">
              <v-card
                ...
                class="col-md-2 offset-md-1 my-2"
              >
                <img :src="ropa.img" alt="" class="img-fluid mx-auto" />
                <v-card-title class="text-center">{{ ropa.name }}</v-card-title>
                <v-card-text v-if="ropa.price > 0">
                  <strong class="text-danger">
                    Precio por unidad: $ {{ ropa.price }}
                  </strong>
                </v-card-text>
                <v-card-actions>
                  <div v-if="!ordenesHas(ropa)">
                    <button class="btn btn-light" @click="ordenesAdd(ropa)">
                      Agregar
                    </button>
                  </div>
      ...
      <v-dialog v-model="dialog" persistent>
        ...
        <v-card v-if="!paying">
          ...
          <v-card-text class="text-servicios">
            La cobertura de nuestro servicio de Retiro y Entrega a domicilio es
            gratuito para la denominada zona urbana de Castro. Para servicios
            fuera de la zona urbana de Castro, cont&aacute;ctenos (llamado o
            WhatsApp) al <b>+56 9 5699 3082</b> o bien al correo
            <a href="mailto:contacto@lavatu.cl"><b>contacto@lavatu.cl</b></a>
          </v-card-text>
          <v-card-text class="text-servicios">
            LavaT&uacute; no presta sus servicios para lavado y secado de ropa
            de mascotas.
          </v-card-text>
          <v-card-text class="text-servicios">
            LavaT&uacute; no realiza el desmanchado en ning&uacute;n tipo de
            ropa. El proceso de desmanchado de ropa es de exclusiva
            responsabilidad del cliente.
          </v-card-text>
          <v-card-text class="text-servicios">
            Sabemos que el tiempo de nuestros clientes es invaluable por lo cual
            intentamos responder a sus requerimientos en el menor tiempo
            posible. Hemos dise&ntilde;ado un horario de retiro y entrega con el
            prop&oacute;sito que nuestros clientes planifiquen, aprovechen y
            disfruten de su tiempo.
          </v-card-text>
          <v-card-text class="text-servicios">
            Horario de retiros y entregas:
            <b>
              De lunes a viernes desde las 10:00 horas hasta las 18:00 horas
            </b>
          </v-card-text>
          <v-card-text class="text-servicios">
            El plazo de entrega depender&aacute; de la magnitud del servicio
            pero nuestros esfuerzos est&aacute;n dirigidos a poder entregar la
            ropa al d&iacute;a h&aacute;bil siguiente de haber sido retirada del
            domicilio de nuestro cliente.
          </v-card-text>
          <v-card-text class="text-servicios">
            Una vez que el cliente finalice la solicitud de servicio, a la
            brevedad nos pondremos en contacto para coordinar el retiro.
            As&iacute; tambi&eacute;n, una vez que la ropa est&eacute; ya lista
            (lavada y secada), nos pondremos en contacto con nuestro cliente
            para confirmar el horario de entrega con el prop&oacute;sito de
            asegurarnos que haya alguna persona en la residencia del cliente que
            reciba la ropa.
          </v-card-text>
          <v-card-text class="text-servicios">
          ...
          <v-card-text class="text-servicios">
            El ciclo de Lavado y Secado es por carga de ropa. El tama&ntilde;o
            de la carga se determina m&aacute;s por el volumen de la ropa que
            por su peso. Como referencia, se considera que una carga de ropa es
            aproximadamente 8 Kg a 8,5 Kg. Sobrecargar la m&aacute;quina
            lavadora con ropa conlleva a que no obtendremos un buen resultado
            del lavado de la ropa.
          </v-card-text>
          <v-card-text class="text-servicios">
            Una vez recepcionada la ropa y antes de ponerla en la lavadora,
            &eacute;sta podr&iacute;a ser separada en cargas adicionales si
            alguna de las siguientes situaciones sucede:
            <ul>
            ...
            En caso de ocurrir alguna de las situaciones previamente descritas,
            contactaremos al cliente para requerir su autorizaci&oacute;n con el
            objetivo de procesar cargas de ropa adicionales.
          </v-card-text>
          <v-card-title class="headline">
            Recomendaciones generales respecto del env&iacute;o de ropa
          </v-card-title>
          <v-card-text class="text-servicios">
            <ul>
              <li>
                Siempre revisar las etiquetas de la ropa con las recomendaciones
                del fabricante respecto de lavado y secado.
              </li>
              <li>
                Aconsejamos:
                <ul>
          ...
          <v-card-actions>
            ...
            <v-btn color="green darken-2" text @click="paying = true">
              Acepto Condiciones del Servicio, Precio y Recomendaciones
            </v-btn>
            <v-spacer></v-spacer>
          </v-card-actions>
        </v-card>
        <v-card v-else>
          <v-card-title class="headline">Detalle de tu pago</v-card-title>
          <v-card-text>
